...
def follow_wall_config():
    global findWallLimit, follow_wall_direction, wall_dist_limit, wall_dist_limit_min, k_front_outer_right, k_front_center_right, k_front_center, k_front_center_left, k_front_outer_left, front_prox_percent, calibration, iniLW, iniRW, speed
    avoid_obstacles_config()
    follow_wall_direction = -1 # 1: LEFT, -1: RIGHT
    wall_dist_limit_min = 150
    wall_dist_limit = 500
...
def race_config():
    global MAX_SPEED, speed, findWallLimit, wall_dist_limit, wall_dist_limit_min, k_front_outer_right, k_front_center_right, k_front_center, k_front_center_left, k_front_outer_left, front_prox_percent, calibration, iniLW, iniRW, corner, innercorner
    avoid_obstacles_config()
...
def distances_follow_wall(laser, wall):
    go_left = 0
    res = 300
    if laser.front_center.dist < 100:
        neato.rotate(-follow_wall_direction*90)
        return 0, 0
    else:
        if wall.dist < 75:
            go_left = -follow_wall_direction*40
        elif wall.dist > 100:
            go_left = follow_wall_direction*150
        #go_left = -follow_wall_direction*(1/(wall.proximity_percent() - 0.2)*80) # -1 * (1/1 - 0.1) * 80
        rL, rR = neato.rotation_motors(wall.sector.nearest().alfa - follow_wall_direction*90)
        debug("go_left, rL, rR, proximity: %.2f, %.2f, %.2f, %.2f" % (go_left, rL, rR, wall.proximity_percent()))
        return 1.5*rL + res, 1.5*rR + go_left + res
...
def distances_race(laser, wall_sector, inner=False):
    global corner, innercorner
    go_right = go_left = 0
    left = laser.front_center_left
    debug("Left: %s" % str(left))
    debug("Distance to wall: %i" % left.dist)
    detected_wall = left.dist <= 550
    if not corner and not detected_wall:
        corner = True
        debug("adjusting direction")
        if left.dist == 0:
            neato.set_motors(left=50,right=50)
            neato.sleep(50/250)
        go_right, go_left = neato.rotation_motors(90)
        dL = 250 + go_right
        dR = 250 + go_left
    else:
        corner = False
        debug("go straight and avoiding obstacles")
        if inner and not innercorner and laser.front_center.proximity_percent() > 0.7:
            debug("innercorner")
            innercorner = True
            go_right, go_left = neato.rotation_motors(-90)
            dL = 150 + go_right
            ...
            alfa = left.sector.nearest().alfa
            debug("straight %i " % alfa)
            innercorner = False
            dL, dR = neato.rotation_motors(alfa - 54)
            dL = dL + 400
            dR = dR + 400
    return dL, dR
...
def race_inner(laser):
    dL, dR = distances_race(laser, scan_wall_race_direction(), inner=True)
    neato.set_motors(left=dL, right=dR, speed=speed)
